...
export class MembershipTypeComponent implements OnInit {
	...
	refreshCountries() {
      ...
      this._member.get_MemberShip().subscribe({
	next : (res) => {
		console.log(res)
		this.allMembership = res;
		this.collectionSize = res.length
		this.countries = res.map((item: any, i: number) => ({ id: i + 1, ...item }))
		.slice(
	...
	error : (err) => {
		console.log('err in get Membership')
	},
})
    ...
    openModal() {
		this.modalService.open(AddMembershipTypeComponent,{ size: 'xl' });
